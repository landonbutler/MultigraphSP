...
class Model:
    ...
    def __init__(self,
        ...
        if penalty is not None:
            ...
            assert penalty[0] in dir(self.archit) # That the function is part of
                # the architecture
            self.penalty = penalty[0]
            self.penaltyMultiplier = penalty[1]
        else:
            self.penalty = None
    ...
    def __repr__(self):
        ...
        if self.penalty is not None:
            reprString += "\n"
            reprString += "Penalty: %s, Multiplier: %.4f" % (self.penalty,
