...
class Trainer:
    ...
    def trainBatch(self, thisBatchIndices):
        ...
        # See if there's a penalty
        if self.model.penalty is not None:
            thisPenalty = eval('self.model.archit.%s' % self.model.penalty)
            penaltyValue = thisPenalty()
            lossValueTrain = lossValueTrain + \
                                     self.model.penaltyMultiplier * penaltyValue
...
class TrainerSingleNode(Trainer):
    ...
    def trainBatch(self, thisBatchIndices):
        ...
        # Obtain the output of the GNN
        yHatTrain, _ = self.model.archit.singleNodeForward(xTrain, targetIDs)
    ...
    def validationStep(self):
        ...
        with torch.no_grad():
            # Obtain the output of the GNN
            yHatValid, _ = self.model.archit.singleNodeForward(xValid,targetIDs)
