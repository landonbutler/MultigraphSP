...
def evaluateSingleNode(model, data, **kwargs):
    ...
    with torch.no_grad():
        # Process the samples
        yHatTest, yHatGNN = model.archit.singleNodeForward(xTest, targetIDs)
        # yHatTest is of shape
        #   testSize x numberOfClasses
        # We compute the error
        costBest = data.evaluate(yHatTest, yTest)
    ...
    evalVars = {}
    evalVars['costBest'] = costBest.item()
    evalVars['yHatBest'] = yHatTest.data.cpu().numpy()
    evalVars['yGNNbest'] = yHatGNN.data.cpu().numpy()
    ...
    evalVars['costLast'] = costLast.item()
    evalVars['yHatLast'] = yHatTest.data.cpu().numpy()
    evalVars['yGNNlast'] = yHatGNN.data.cpu().numpy()
