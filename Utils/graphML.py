...
LSIGF_DB: Applies a delayed linear shift-invariant graph filter
GRNN_DB: Computes the sequence of hidden states
...
class MultiGraphFilter(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        F = self.f_out
        G = self.f_in
        N = self.GSOs[0].shape[-1]  # number of nodes
...
class GraphFilter_DB(nn.Module):
    """
    GraphFilter Creates a (linear) layer that applies a graph filter (i.e. a
